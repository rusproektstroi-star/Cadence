...
| VM shows as running, Tools alive, but answers neither SSH nor ping | **VMware was not running on its own hypervisor.** Windows' hypervisor (pulled in by WSL2, Docker Desktop, Windows Sandbox or Core Isolation's Memory Integrity) had taken the CPU's virtualization, so Workstation was relegated to **ULM** mode — visible in every `vmware.log` as `Monitor Mode: ULM` plus `Syncing WHP TSCs` lines. In that mode guest timers are emulated, and they run away: the local APIC timer climbs from ~100 to 8200 interrupts per second per core, every vCPU burns in the interrupt handler, and the guest stops answering **56–61 minutes after boot** — nine boots across three machines, including an untouched stock Ubuntu that never saw our image. **Cure, one line in an elevated PowerShell on the host: `bcdedit /set hypervisorlaunchtype off`, then reboot.** Afterwards `vmware.log` must read `Monitor Mode: CPL0` with no WHP lines. Cost: WSL2, Docker Desktop and Windows Sandbox stop working — they and VMware cannot share hardware virtualization. Undo with `bcdedit /set hypervisorlaunchtype auto`. `docs/OPERATIONS.md` §7 has the measurements, the eleven wrong answers we went through first, and the measurement traps that cost us hours. |

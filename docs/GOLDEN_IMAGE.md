...
**Раздувание памяти (balloon) — выключить в `.vmx` образа**, иначе гипервизор будет отбирать память
у работающего гостя даже при свободной памяти на хосте, и гость встанет намертво (ни OOM, ни следа в
логах — только лавина `vmballoon_work` в журнале; разбор — `docs/OPERATIONS.md` §7). Дописать в
`.vmx` образа на выключенной машине, клоны унаследуют:
...
Условие применимости: физической памяти хоста хватает, чтобы держать все поднятые гости целиком.
Если это не так — не закреплять память, а поднимать меньше машин одновременно.

**Ядер образа — от 4.** Их должно быть больше, чем одновременно работающих сессий агентов: на двух
ядрах машина доходит до отказа заметно быстрее. Лекарством от зависаний это НЕ является: 20.09 та же машина встала и на четырёх ядрах при нагрузке 1.48 — разбор в `docs/OPERATIONS.md` §7.
Задаётся в `.vmx` образа, клоны наследуют: `numvcpus = "4"`.
...
# Фоновые обновления выключаются на образе, а не на каждом клоне: тяжёлая работа в фоне на машине
# с двумя сессиями агентов не нужна и мешает воспроизводимости (при разборе зависаний гостя
# apt-daily-upgrade сперва выглядел причиной — ею он не оказался, см. docs/OPERATIONS.md §7, но
# отключение осталось как осмысленное). Обновлять пакеты — руками и осознанно.

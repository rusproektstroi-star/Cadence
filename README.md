...
Единственные шаги, которые агент не делает сам: добавление deploy-ключа в настройках
GitHub-репозитория, интерактивный вход `claude` в панелях и правка системного `hosts` (UAC). Они
помечены в документации как «нужен человек», без попыток обойти.

Обычные операции на гостях пароля **не требуют**: образ раздаёт без пароля узкий список команд
(`netplan apply` и пути `/etc/netplan/*`, `systemctl`, `apt-get`/`apt`). Проверять списком —
`ssh <id> "sudo -n -l"`, а не пробным `sudo -n true`: `true` в список не входит, и такая проверка
даёт ложный вывод «нужен пароль» (на этом уже спотыкались).
...
docs/NEW_MACHINE_CHECKLIST.md — чек-лист развёртывания проектной машины, проходить целиком

...
![Оркестратор — самопредставление и статус парка](docs/images/orchestrator-status.png)
...
![Кросс-ролевая маршрутизация между панелями агентов](docs/images/cross-role-demo.png)

`devpanel` — ручная панель нескольких dev-серверов проекта, адрес одной строкой `IP:порт` для
прямой вставки в браузер:

![devpanel — таблица dev-серверов](docs/images/devpanel-table.png)
...
![mc — файловый менеджер](docs/images/mc-file-browser.png)
...
Cadence распространяется под **[SESL](https://sesl.io) (Shared Effort Software License)**,
конфигурация с весовым консенсусом при нескольких участниках и индивидуальным авторством по
вкладам — полный текст в [`LICENSE.html`](LICENSE.html) (сертификат с проверяемым License ID) и
кратко ниже.
...
одной машины оператора, через несколько вкладок терминала.

Перенос кода/канона оркестратора из рабочего окружения в этот репозиторий, с обобщением частных
деталей (сеть конкретного хоста, имена конкретных проектов) под конфигурируемые примеры — в
процессе.
